(function () {
  ...
  async function downloadJsonFile(url, filename) {
    ...
    try {
      ...
      downloadMsg.textContent = 'Download failed: ' + e.message;
    }
  }
  ...
  document.getElementById('btnDownloadTrain').addEventListener('click', function () {
    ...
    downloadJsonFile(
      '/phase_identification_benchmark/training-phase-data?difficulty=' + encodeURIComponent(diff),
      'training_phase_data_' + diff + '.json'
    );
  });

  document.getElementById('btnDownloadPhase').addEventListener('click', function () {
    var diff = difficultySelect.value;
    downloadJsonFile(
      '/phase_identification_benchmark/phase-data?difficulty=' + encodeURIComponent(diff),
      'phase_data_' + diff + '.json'
    );
  });
  ...
  ['dragenter', 'dragover'].forEach(function (ev) {
    guessDrop.addEventListener(ev, function (e) { e.preventDefault(); guessDrop.classList.add('is-over'); });
  });
  ['dragleave', 'dragend'].forEach(function (ev) {
    guessDrop.addEventListener(ev, function (e) { e.preventDefault(); guessDrop.classList.remove('is-over'); });
  });
  guessDrop.addEventListener('drop', function (e) {
    e.preventDefault(); guessDrop.classList.remove('is-over');
    if (e.dataTransfer && e.dataTransfer.files.length) {
      guessFile.files = e.dataTransfer.files;
      onFileSelected();
    }
  });
  guessFile.addEventListener('change', onFileSelected);

  function onFileSelected() {
    var f = guessFile.files[0];
    guessLabel.textContent = f ? f.name : 'No file selected';
  ...
  btnSubmit.addEventListener('click', async function () {
    ...
    try {
      ...
      var guesses;
      try { guesses = JSON.parse(text); } catch (e) { throw new Error('Invalid JSON: ' + e.message); }

      var res = await fetch('/phase_identification_benchmark/submit-results', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ guess_id: user, grid_id: grid, guesses: guesses })
      });
      ...
      submitResultMsg.textContent = 'Predictions for grid "' + grid + '" submitted as user "' + user + '".';
      document.getElementById('scSubmission').textContent = 'Submitted';
    } catch (e) {
      uploadMsg.textContent = 'Error: ' + e.message;
    } finally {
      btnSubmit.disabled = !guessFile.files.length;
    }
  });
  ...
  btnScore.addEventListener('click', async function () {
    ...
    if (!user || !grid) {
      ...
      scoreErrorMsg.textContent = 'Please enter both Submission ID and Grid ID in Benchmark Setup.';
      return;
    }
    ...
    try {
      var res = await fetch('/phase_identification_benchmark/score?guess_id=' + encodeURIComponent(user) + '&grid_id=' + encodeURIComponent(grid));
      if (!res.ok) throw new Error(await res.text());
      var data = await res.json();

      scoreResult.style.display = 'block';
      scoreTable.innerHTML =
        '<tr><th>User</th><td>' + data.guess_id + '</td></tr>'
      ...
      scoreError.style.display = 'block';
      scoreErrorMsg.textContent = e.message;
    } finally {
